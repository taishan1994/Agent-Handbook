...
class Flow(BaseNode):
    ...
    def get_next_node(self,curr,action):
        # 修复：确保 action 是可哈希类型
        if not isinstance(action, (str, int, float, bool, type(None))):
            action = "default"
...
class AsyncFlow(Flow,AsyncNode):
    ...
    async def post_async(self,shared,prep_res,exec_res):
        # 修复：确保返回值总是可以安全处理
        # 即使exec_res是字符串，也直接返回，不尝试调用get方法
        return exec_res
...
# 确保类可以被正确导入
__all__ = ['BaseNode', 'Node', 'BatchNode', 'Flow', 'BatchFlow', 'AsyncNode', 'AsyncBatchNode', 'AsyncParallelBatchNode', 'AsyncFlow', 'AsyncBatchFlow', 'AsyncParallelBatchFlow']