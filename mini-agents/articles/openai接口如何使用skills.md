# 什么是skills
可参考：https://support.claude.com/en/articles/12512176-what-are-skills

一句话总结：skills是包含指令、脚本和资源的文件夹。

# 如何构建一个skill
这里我们以pdf的读取来进行展示如何构建一个skill。一般skill由以下两个部分组成：
```shell
--skill目录：
----script：能够执行的一些脚本
----SKILL.md：skill的使用说明
```
需要注意的是在SKILL.md可以定义去另外的md下了解其它信息，比如reference.md。

在tools/下新建一个skill_loader.py，用于读取skills下的skill并进行加载。
